...
/// Token 17 / set_constant pins the singleton. Retraction ignores the
/// increment: the point stays at the start and is not sphere-normalized.
#[test]
fn c_abi_constant_stays_on_the_set() {
    ...
    assert_eq!(st, rgmin_status_t::RGMIN_SUCCESS);
    assert_eq!(x.len(), 3);
    assert!((x[0] - start[0]).abs() < 1e-15);
    assert!((x[1] - start[1]).abs() < 1e-15);
    assert!((x[2] - start[2]).abs() < 1e-15);
    let fro = x.iter().map(|a| a * a).sum::<f64>().sqrt();
    assert!((fro - 1.0).abs() > 0.5, "must not be the sphere {x:?}");
