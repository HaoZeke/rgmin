...
unsafe extern "C" fn sphere_cplx_lin_eval(
    _user: *mut c_void,
    x: *const DLManagedTensorVersioned,
    value_out: *mut f64,
) -> rgmin_status_t {
...
unsafe extern "C" fn sphere_cplx_lin_grad(
    _user: *mut c_void,
    x: *const DLManagedTensorVersioned,
    g: *mut DLManagedTensorVersioned,
) -> rgmin_status_t {
...
/// Token 20 / set_sphere_complex keeps the interleaved C^n packing
/// on the unit sphere. Retraction is normalize(x+v), not pair-wise
/// S^1. Tokens 7-10 unused.
#[test]
fn c_abi_sphere_complex_stays_on_the_set() {
    ...
    let st = unsafe {
        rgmin_solver_step(
            session,
            Some(sphere_cplx_lin_eval),
            Some(sphere_cplx_lin_grad),
            std::ptr::null_mut(),
            xt,
            &mut out,
        )
