...
/// Token 21 / set_positive stays in the positive orthant.
/// Retraction is x .* exp(v ./ x), not sphere-normalization.
/// Reserved tokens 7-10 unused.
#[test]
fn c_abi_positive_stays_on_the_set() {
    ...
    unsafe { rgmin_tensor_free(xt) };
    assert_eq!(st, rgmin_status_t::RGMIN_SUCCESS);
    assert!(x.iter().all(|&xi| xi > 0.0), "left the positive orthant {x:?}");
    assert!(
        (x[0] - 1.0).abs() > 1e-8,
        "expected a retraction step {x:?}"
    );
