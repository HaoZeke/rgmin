...
#define xts_solver_set_se_n rgmin_solver_set_se_n
 ...
 *  SO3 is length 9; SE3 is length 12. SE(n) is n-by-n then t via
 *  xts_solver_set_se_n. Oblique is n-by-m via
...
/** SE(n), n >= 2. Packed row-major R then t, length n*n+n. Distinct from SE3. */
void xts_solver_set_se_n(xts_solver_t *solver, size_t n);
