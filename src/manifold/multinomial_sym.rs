...
//! `V - (alpha 1^T + 1 alpha^T) ⊙ X` (Douik and Hassibi, arXiv:1802.02628).
//! Retraction is `X ⊙ exp(V ⊘ X)`, Sinkhorn, then symmetrize.
//! Transport is projection at the arrival point. `egrad2rgrad` is
//! `(egrad - alpha 1^T - 1 alpha^T) ⊙ X` with
//! `(I + X) alpha = (X ⊙ egrad) 1`.
//!
...
impl MultinomialSymmetric {
    ...
    fn apply_ix(&self, x: &Array2<f64>, z: &Array1<f64>) -> Array1<f64> {
        let n = self.n;
        let mut out = z.clone();
        for i in 0..n {
            out[i] += vecops::dot(x.row(i), z.view());
        }
        out
    }

    fn solve_ix(&self, x: &Array2<f64>, b: &Array1<f64>) -> Array1<f64> {
        let n = self.n;
...
impl Manifold for MultinomialSymmetric {
    ...
    fn project(&self, x: &Array1<f64>, v: &Array1<f64>) -> Array1<f64> {
        ...
        let xm = self.symmetrize(self.mat(x));
    ...
    fn egrad2rgrad(&self, x: &Array1<f64>, egrad: &Array1<f64>) -> Array1<f64> {
        ...
        let xm = self.symmetrize(self.mat(x));
        let gm = self.symmetrize(self.mat(egrad));
        ...
        for i in 0..n {
            ...
            mu[i] = vecops::sum(row.view());
        }
        let alpha = self.solve_ix(&xm, &mu);
        let mut out = Array2::<f64>::zeros((n, n));
        for i in 0..n {
            for j in 0..n {
                out[(i, j)] = (gm[(i, j)] - alpha[i] - alpha[j]) * xm[(i, j)];
            }
        }
        self.pack_mat(&out)
    }
    ...
    fn retract(&self, x: &Array1<f64>, v: &Array1<f64>) -> Array1<f64> {
        ...
        let mut y = self.sinkhorn(self.symmetrize(y));
        for i in 0..n {
            for j in 0..n {
                y[(i, j)] = y[(i, j)].max(f64::EPSILON);
            }
        }
        self.pack_mat(&self.symmetrize(y))
    }
...
mod tests {
    ...
    fn transport_is_projection_at_arrival() {
        let m = MultinomialSymmetric { n: 2 };
        let x = barycenter(2);
        let y = m.retract(&x, &array![0.0, 0.05, 0.05, -0.1]);
        let v = array![0.1, -0.05, -0.05, 0.0];
    ...
    fn egrad2rgrad_is_fisher() {
        let m = MultinomialSymmetric { n: 2 };
        let x = barycenter(2);
        let egrad = array![1.0, 2.0, 2.0, 4.0];
