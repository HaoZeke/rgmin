...
pub enum rgmin_manifold_t {
    ...
    /// Reserved: 7 SPD, 8 Grassmann, 9 Hyperbolic, 10 Poincare, 15 SO(n).
    ...
    /// Reserved: 15 SO(n).
    /// SE(n), n >= 2, row-major R then t, length n^2+n.
    /// Shape from `rgmin_solver_set_se_n`. Token defaults to n = 2.
    /// Distinct from RGMIN_MANIFOLD_SE3.
    RGMIN_MANIFOLD_SE_N = 16,
}
...
pub unsafe extern "C" fn rgmin_solver_set_manifold(
    ...
    let kind = match manifold {
        ...
        rgmin_manifold_t::RGMIN_MANIFOLD_SE_N => ManifoldKind::SeN { n: 2 },
...
/// SE(n), n >= 2. Packed row-major R then t, length n*n+n. Not SE(3)'s token.
/// A 3N cluster is not this packing.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rgmin_solver_set_se_n(solver: *mut rgmin_solver_t, n: usize) {
    if solver.is_null() {
        return;
    }
    unsafe { (*solver).solver.set_se_n(n) };
}
