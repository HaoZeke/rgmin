...
pub enum rgmin_manifold_t {
    ...
    /// SO(n), n >= 2, row-major n². Shape from `rgmin_solver_set_so_n`.
    /// Token defaults to n = 2. Distinct from RGMIN_MANIFOLD_SO3.
    RGMIN_MANIFOLD_SO_N = 15,
}
...
pub unsafe extern "C" fn rgmin_solver_set_manifold(
    ...
    let kind = match manifold {
        ...
        rgmin_manifold_t::RGMIN_MANIFOLD_SO_N => ManifoldKind::SoN { n: 2 },
...
/// SO(n), n >= 2. Packed row-major, length n*n. Not SO(3)'s token.
/// A 3N cluster is not this packing.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rgmin_solver_set_so_n(solver: *mut rgmin_solver_t, n: usize) {
    if solver.is_null() {
        return;
    }
    unsafe { (*solver).solver.set_so_n(n) };
}
