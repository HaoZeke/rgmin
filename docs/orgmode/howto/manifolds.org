...
| ~EuclideanComplex~ | interleaved ~(re, im)~, even length ~>= 2~ | ~x + v~ |
...
- ~EuclideanComplex~ is manopt ~euclideancomplexfactory~:
  interleaved ~(re, im)~ pairs, even length ~>= 2~. The geometry
  is Euclidean. It is not ~ComplexCircle~ and not the sphere.
- ~Constant~ is manopt ~constantfactory~: a single fixed point of
  length ~n >= 1~. Projection and transport are zero. Retraction
  returns ~x~. It is a product factor, not a constraint solver.
