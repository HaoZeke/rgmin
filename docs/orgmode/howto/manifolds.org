...
- ~Euclidean~ is manopt ~euclideanfactory~ on a rank-1 vector of
  length ~n~. Projection and transport are the identity.
  Retraction is ~x + v~. The inner product is the Frobenius
  Euclidean dot via ~vecops~. Typical distance is ~sqrt(n)~.
  Matrix or tensor packing (~euclideanfactory(m, n)~ / a size
  vector) is a child ticket.
