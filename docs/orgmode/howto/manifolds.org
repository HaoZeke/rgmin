...
| ~Unitary { n }~ | interleaved ~(re, im)~ row-major, length ~2 n^2~ | complex QR of ~X + V~ |
...
~Sphere~, ~So3~, ~Stiefel~, ~Se3~, ~ComplexCircle~, and ~Unitary~
are matrix-manifold embeddings. ~So3~ rejects any length other
than 9. ~Se3~ rejects any length other than 12. ~ComplexCircle
{ n }~ rejects any length other than ~2 n~. ~Unitary { n }~
rejects any length other than ~2 n^2~. They do not pack or
...
- ~Unitary { n }~ is manopt ~unitaryfactory(n)~: an ~n x n~
  ...
  row-major, length ~2 n^2~. Projection is
  ~U skewh(U^* Z)~ (ambient tangent). Retraction is thin QR
  of ~X + V~ with a real-positive diagonal. Transport is
  projection at the arrival point. It is not the sphere and
  not a 3N cluster.
