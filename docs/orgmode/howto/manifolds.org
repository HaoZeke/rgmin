...
| ~Constant { n }~ | singleton, length ~n~ | ~x~ (ignore ~v~) |
...
~EuclideanComplex~ are matrix-manifold embeddings. ~So3~ rejects
any length other than 9. ~Se3~ rejects any length other than 12.
~ComplexCircle { n }~ and ~EuclideanComplex { n }~ reject any
...
- ~EuclideanComplex { n }~ is manopt ~euclideancomplexfactory(n)~:
  ...
  stay reserved.
- ~Constant { n }~ is manopt ~constantfactory~: the singleton
  ~{A}~ of packed length ~n~. Projection and transport are the
  zero vector. Retraction stays at the point. It is not the
  sphere, not Euclidean, and not a 3N cluster packing. C token 17
  (~rgmin_solver_set_constant~). Tokens 7-10 stay reserved.
