...
- ~Stiefel~ is manopt ~stiefelfactory(n, 1)~: a unit column,
  ...
  (~\sqrt{p k}~ at ~p = 1~, ~k = 1~), not ~pi~. ~StiefelP { n, p }~
  / ~set_stiefel(n, p)~ for ~p > 1~ is a different token:
  column-major ~n p~, projection ~U - X sym(X^T U)~, QR
  retraction. A 3N cluster is ~RigidQuotient~, not this packing.
