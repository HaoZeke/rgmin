...
- ~SphereComplex { n }~ is manopt ~spherecomplexfactory~: the unit
  sphere in \(\mathbb{C}^n\), interleaved ~(re, im)~, length ~2 n~.
  Projection is ~v - Re(x^* v) x~. Retraction is ~(x+v)/||x+v||~.
  Transport is projection at arrival. It is not the real sphere,
  not ~(S^1)^n~, and not a 3N cluster. C token 20
  (~rgmin_solver_set_sphere_complex~). Tokens 7-10 stay reserved.
