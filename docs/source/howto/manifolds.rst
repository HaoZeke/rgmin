...
- ``Positive { n }`` is manopt ``positivefactory(n)``: the positive
  orthant of packed length ``n``. Projection is the identity.
  Retraction is ``x .* exp(v ./ x)``. Transport is the identity.
  It is not the sphere, not SPD, and not a 3N cluster. C token
  21 (``rgmin_solver_set_positive``). Tokens 7-10 stay reserved.
