...
- ``MultinomialSymmetric { n }`` is manopt
  ...
  Retraction is ``X ⊙ exp(V ⊘ X)`` then Sinkhorn then
  symmetrize. Transport is projection at arrival. It is not the
  simplex, not the (asymmetric) Birkhoff polytope, not the sphere,
  and not a 3N cluster. C token 19
