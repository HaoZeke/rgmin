...
Positive orthant (``positive`` / ``RGMIN_MANIFOLD_POSITIVE`` = 19):
manopt ``positivefactory``, retraction ``x odot exp(v oslash x)``.
Real phase (``realphase`` / ``RGMIN_MANIFOLD_REALPHASE`` = 20): product
of 0-spheres, retraction stays on ``{+/-1}^n``. Centered matrices
(``centered`` / ``RGMIN_MANIFOLD_CENTERED`` = 21): doubly-centered
``m x n``, row-major ``m n``, ``rgmin_solver_set_centered``. Stamp 1.15.5.
...
``(S^1)^n``, positive token 19, realphase token 20, centered
token 21. Reserved: 7 SPD, 8 Grassmann, 9 Hyperbolic, 10
Poincare, 15 SO(n), 16 SE(n), 17-18. Hourglass mark and the
