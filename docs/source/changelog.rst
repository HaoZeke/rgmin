...
without the ``highs`` feature). ``rgmin_solver_set_trust`` is the
L\ :sub:`inf`\ radius; ``rgmin_solver_add_equality`` /
...
is the centering operator (``X 1 = 0`` by default, or
``1^T X = 0``), retraction is ``X + U`` then center, transport is
...
identity. Not the sphere and not a 3N cluster. Tokens 7-10 stay
reserved. ``abi_minor`` 20.
...
``n >= 2``. Fisher projection, Sinkhorn+symm retraction, transport
at arrival. A 3N cluster is rejected. Tokens 7-10 stay reserved.
``abi_minor`` 18.
